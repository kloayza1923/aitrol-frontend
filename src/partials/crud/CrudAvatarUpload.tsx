...
const CrudAvatarUpload = ({
  ...
  setAvatar: (avatar: IImageInputFile[]) => void;
  onChange: (event: React.ChangeEvent<HTMLInputElement>) => void;
  onSuccess: (file: IImageInputFile) => void;
  value?: string;
}) => {
  useEffect(() => {
    console.log('CrudAvatarUpload value changed:', value);
    if (value) {
      console.log('Setting avatar from value:', value);
  ...
  return (
    <ImageInput
      ...
      onChange={(selectedAvatar) => {
        ...
        onChange({ target: { name: 'foto', value: selectedAvatar[0]?.dataURL } });
        if (selectedAvatar.length > 0) {
          onSuccess(selectedAvatar[0]);
        }
      }}
      inputProps={{
        id: 'image_input',
        'aria-describedby': 'image_input_tooltip'
      }}
    >
      {({ onImageUpload }) => (
        <div className="image-input size-16" onClick={onImageUpload}>
          ...
          >
            {avatar.length > 0 && <img src={avatar[0].dataURL} id="avatar_input" alt="avatar" />}
