...
import useLoadUserMenu from '@/hooks/useLoadUserMenu';
...
export interface IDemo1LayoutProviderProps {
  ...
  menuLoading: boolean; // Tracks if the user menu is still loading
...
const initalLayoutProps: IDemo1LayoutProviderProps = {
  ...
  menuLoading: true, // Menu is loading by default
...
const Demo1LayoutProvider = ({ children }: PropsWithChildren) => {
  ...
  const { isLoading: menuLoading } = useLoadUserMenu(usuarioId);
