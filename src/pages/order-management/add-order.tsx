...
const AddOrder = () => {
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      const res = await FetchData('ordenes', 'POST', payload);
      if (res?.detail) {
                              ...
                              <Grid item xs={12} md={6}>
                                ...
                                >
                                  ...
                                  {/*  <MuiSelect
                                    ...
                                    onChange={(e) => {
                                      setFieldValue(
                                        `vehiculos.${idx}.vehiculo_id`,
                                        Number(e.target.value)
                                      );
                                      console.log(
                                        'Selected vehicle id:',
                                        vehicles.find((veh) => veh.id === Number(e.target.value))
                                      );
                                      const conductor = vehicles.find(
                                        (veh) => veh.id === Number(e.target.value)
                                      )?.conductor_id;
                                  ...
                                  <Autocomplete
                                    ...
                                    getOptionLabel={(option: any) =>
                                      option.nombre +
                                      ' - ' +
                                      (option.conductor_nombre || option.tipo_nombre || '')
                                    }
                                    value={
                                      vehicles.find((veh) => veh.id === Number(v.vehiculo_id)) ||
                                      null
                                    }
                                    onChange={(_, value) => {
                                      setFieldValue(
                                      ...
                                      const conductor = value ? value.conductor_id : '';
                                      setFieldValue(`vehiculos.${idx}.conductor_id`, conductor);
                                    }}
                                    renderInput={(params) => (
                                      <TextField
