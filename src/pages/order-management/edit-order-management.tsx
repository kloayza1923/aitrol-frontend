...
const EditOrdenManagement = () => {
  ...
  const get_ordenes = async () => {
    ...
    if (!id) return;
    const data = await FetchData('ordenes/' + id, 'GET', null);
    if (data?.detail) {
      toast.error(data.detail);
      return;
    }
    setOrder(data);
    // fetch vehiculos for the order
    get_ordenes_vehiculos(id);
  };

  const get_ordenes_vehiculos = async (id: any) => {
    const data = await FetchData('ordenes/vehiculos/' + id, 'GET', null);
    if (data?.detail) {
      toast.error(data.detail);
      return;
    }
    // ensure the shape matches Formik (numbers where expected)
    const mapped = (data || []).map((v: any) => ({
      vehiculo_id: v.vehiculo_id ?? '',
      conductor_id: v.conductor_id ?? '',
      vueltas: v.vueltas ?? 0
    }));
    setVehiclesArray(mapped);
  };
  ...
  // NOTE: intentionally not listing fetch helpers as dependencies to avoid repeated calls
  ...
  const schema = Yup.object().shape({
    ...
    fecha: Yup.string()
      ...
      .test('fecha-futura', 'La fecha no puede ser pasada', (value) => {
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      const res = await FetchData('ordenes/' + orderid, 'PUT', payload);
      if (res?.detail) {
  ...
  // enable reinitializing form after fetching order and vehicles
  const combinedInitials = { ...initialValues } as any;
  if (order) {
    combinedInitials.cliente_id = order.cliente_id ?? '';
    combinedInitials.peso_total = order.peso_total ?? '';
    combinedInitials.unidad_id = order.unidad ?? '';
    combinedInitials.material_id = order.material_id ?? '';
    combinedInitials.puerto_salida_id = order.puerto_salida_id ?? '';
    combinedInitials.puerto_destino_id = order.puerto_destino_id ?? '';
    combinedInitials.vehiculos_totales = order.vehiculos_totales ?? 0;
    combinedInitials.fecha = order.fecha ?? '';
    combinedInitials.hora_salida = order.hora_salida ?? '';
    combinedInitials.total = order.total ?? '';
    combinedInitials.descripcion = order.descripcion ?? '';
    combinedInitials.modo_entrada = order.modo_entrada ?? '';
    combinedInitials.vehiculos = vehiclesArray.length > 0 ? vehiclesArray : [];

    // Convertir string separado por comas a array de objetos de usuarios
    if (order.orden_usuario_id) {
      const userIds = order.orden_usuario_id.split(',').map((id: string) => parseInt(id.trim()));
      combinedInitials.orden_usuario_id = users.filter((u: any) => userIds.includes(u.id));
    } else {
      combinedInitials.orden_usuario_id = [];
    }
  }
      ...
      <Container>
        <div className="card min-w-full">
          <div className="card-header">
            <h3 className="card-title">Editar una orden</h3>
            <p className="text-sm text-gray-500">Editar la orden de transporte de materiales</p>
          ...
          <Formik
            initialValues={combinedInitials}
                ...
                <Grid container spacing={2}>
                  <Grid item xs={12} md={6}>
                    <FormControl fullWidth error={!!(touched.cliente_id && errors.cliente_id)}>
                      ...
                      <FormHelperText>
                        {touched.cliente_id && errors.cliente_id ? String(errors.cliente_id) : ''}
                      </FormHelperText>
                    </FormControl>
                  </Grid>
                  ...
                  <Grid item xs={12} md={6}>
                    ...
                    >
                      ...
                      />
                      <FormHelperText>
                        {touched.orden_usuario_id && errors.orden_usuario_id
                          ? String(errors.orden_usuario_id)
                          : ''}
                      </FormHelperText>
                    </FormControl>
                  </Grid>
                  ...
                  <Grid item xs={12} md={6}>
                    <TextField
                      ...
                      helperText={
                        touched.peso_total && errors.peso_total ? String(errors.peso_total) : ''
                      }
                    />
                  </Grid>
                  ...
                  <Grid item xs={12} md={4}>
                    <FormControl fullWidth error={!!(touched.unidad_id && errors.unidad_id)}>
                      ...
                      <FormHelperText>
                        {touched.unidad_id && errors.unidad_id ? String(errors.unidad_id) : ''}
                      </FormHelperText>
                    </FormControl>
                  </Grid>
                  ...
                  <Grid item xs={12} md={4}>
                    <FormControl fullWidth error={!!(touched.modo_entrada && errors.modo_entrada)}>
                      ...
                      <FormHelperText>
                        {touched.modo_entrada && errors.modo_entrada
                          ? String(errors.modo_entrada)
                          : ''}
                      </FormHelperText>
                    </FormControl>
                  </Grid>
                    ...
                    >
                      ...
                      <FormHelperText>
                        {touched.puerto_salida_id && errors.puerto_salida_id
                          ? String(errors.puerto_salida_id)
                          : ''}
                      </FormHelperText>
                    </FormControl>
                  </Grid>
                    ...
                    >
                      ...
                      <FormHelperText>
                        {touched.puerto_destino_id && errors.puerto_destino_id
                          ? String(errors.puerto_destino_id)
                          : ''}
                      </FormHelperText>
                    </FormControl>
                  </Grid>
                  ...
                  <Grid item xs={12} md={4}>
                    <TextField
                      ...
                      helperText={
                        touched.vehiculos_totales && errors.vehiculos_totales
                          ? String(errors.vehiculos_totales)
                          : ''
                      }
                    />
                  </Grid>
                  ...
                  <Grid item xs={12} md={4}>
                    <TextField
                      ...
                      helperText={touched.fecha && errors.fecha ? String(errors.fecha) : ''}
                    />
                  </Grid>
                  ...
                  <Grid item xs={12} md={4}>
                    <TextField
                      ...
                      helperText={
                        touched.hora_salida && errors.hora_salida ? String(errors.hora_salida) : ''
                      }
                    />
                  </Grid>
                  ...
                  <Grid item xs={12} md={4}>
                    <TextField
                      ...
                      helperText={touched.total && errors.total ? String(errors.total) : ''}
                    />
                  </Grid>
                  ...
                  <Grid item xs={12}>
                    <TextField
                      ...
                      helperText={
                        touched.descripcion && errors.descripcion ? String(errors.descripcion) : ''
                      }
                    />
                  </Grid>
                </Grid>
                          ...
                          <div key={idx} className="card-table p-4 border rounded">
                            ...
                            <Grid container spacing={2}>
                              <Grid item xs={12} md={6}>
                                <FormControl
                                  ...
                                  error={
                                    !!(
                                      errors.vehiculos &&
                                      (errors.vehiculos as any)[idx] &&
                                      (errors.vehiculos as any)[idx].vehiculo_id
                                    )
                                  }
                                >
                                  ...
                                  <MuiSelect
                                    ...
                                    onChange={(e) => {
                                      ...
                                      setFieldValue(`vehiculos.${idx}.vehiculo_id`, selectedId);
                                      const found = vehicles.find(
                                        (veh: any) => veh.id === selectedId
                                      );
                                      const conductor = found?.conductor_id ?? '';
                                      setFieldValue(`vehiculos.${idx}.conductor_id`, conductor);
                                    }}
                                  >
                                  ...
                                  <FormHelperText>
                                    ...
                                    (errors.vehiculos as any)[idx] &&
                                    (errors.vehiculos as any)[idx].vehiculo_id
                                      ? String((errors.vehiculos as any)[idx].vehiculo_id)
                                      : ''}
                                  </FormHelperText>
                                </FormControl>
                              </Grid>
                                ...
                                <FormControl
                                  ...
                                  error={
                                    !!(
                                      errors.vehiculos &&
                                      (errors.vehiculos as any)[idx] &&
                                      (errors.vehiculos as any)[idx].conductor_id
                                    )
                                  }
                                >
                                  ...
                                  <FormHelperText>
                                    ...
                                    (errors.vehiculos as any)[idx] &&
                                    (errors.vehiculos as any)[idx].conductor_id
                                      ? String((errors.vehiculos as any)[idx].conductor_id)
                                      : ''}
                                  </FormHelperText>
                                </FormControl>
                              </Grid>
                              ...
                              <Grid item xs={12} md={6}>
                                <TextField
                                  fullWidth
                                  name={`vehiculos.${idx}.vueltas`}
                                  label="Vueltas"
                                  type="number"
                                  value={v.vueltas}
                                  onChange={(e) =>
                                    setFieldValue(
                                      `vehiculos.${idx}.vueltas`,
                                      Number(e.target.value)
                                    )
                                  }
                                />
