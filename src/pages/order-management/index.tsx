...
const OrderManagement = () => {
  ...
  const get_url = async () => {
    ...
    if (response) {
      ...
      const message = 'Hola, te comparto esta orden de viaje: ' + response.url;
    ...
    <Fragment>
      <Container>
        <CrudDataGrid
          ...
          endpoint="/ordenes"
          onEditClick={({ id }) =>
            navigateToTab(`/order-edit/${id}`, { customTitle: 'Editar Orden', enableTabs: false })
          }
          onCreateClick={() =>
            navigateToTab('/order-management-add', {
              customTitle: 'Crear Orden',
              enableTabs: false
            })
          }
          buttons={(row) => (
            <>
              ...
              <Tooltip title="Editar orden" arrow>
                <IconButton
                  size="small"
                  onClick={() => navigate(`/order-management-edit/${row.id}`)}
                  sx={{
                    color: 'warning.main',
                    transition: 'all 0.2s ease-in-out',
                    '&:hover': {
