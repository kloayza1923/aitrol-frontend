...
const ClientAddLayout = () => {
  const { currentLayout } = useLayout();
  const location = useNavigate();
  const [avatar, setAvatar] = useState<any>([]);

  const validate_fields = (data: any) => {
    if (!data.nombre || !data.ruc || !data.direccion || !data.telefono || !data.valor_tm) {
  ...
  const create_user = async (e: event) => {
    ...
    const fetchdata = await FetchData('clientes', 'POST', data);
    if (fetchdata.detail) {
