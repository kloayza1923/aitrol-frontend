...
const PuertoEditLayout = () => {
  ...
  const get_client = async () => {
    ...
    const data = await FetchData('puertos/' + id, 'GET', null); // Obtener vehículo desde el backend
    if (data.detail) {
  ...
  const create_user = async (e: event) => {
    ...
    const fetchdata = await FetchData('puertos/' + id, 'PUT', data);
    if (fetchdata.detail) {
      ...
      <Container>
        <div className="card min-w-full">
          <div className="card-header">
            <h3 className="card-title">Editar Puerto</h3>
