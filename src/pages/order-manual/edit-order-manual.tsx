...
const EditOrderManual = () => {
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      const res = await FetchData(`ordenes_manual/${id}`, 'PUT', payload);
      if (res?.detail) {
      ...
      console.error(err);
      toast.error('Error al actualizar la orden manual');
    }
  };
  ...
  const initialValues = {
    ...
    peso_total: orderData?.peso_total || '',
    unidad_id: orderData?.unidad || '',
    material_id: orderData?.material_id || '',
    puerto_salida_id: orderData?.puerto_salida_id || '',
    puerto_destino_id: orderData?.puerto_destino_id || '',
    vehiculos_totales: orderData?.vehiculos_totales || 0,
    fecha: orderData?.fecha || '',
    hora_salida: orderData?.hora_salida || '',
    total: orderData?.total || '',
    descripcion: orderData?.descripcion || '',
    modo_entrada: orderData?.modo_entrada || '',
    orden_usuario_id:
      orderData?.orden_usuario_id
        ?.split(',')
        .map((userId: string) => {
      ...
      <Container>
        <div className="card min-w-full">
          <div className="card-header">
            <h3 className="card-title">Editar Orden Manual #{id}</h3>
            <p className="text-sm text-gray-500">Actualiza la información de la orden manual</p>
                    ...
                    >
                      <Autocomplete
                        ...
                        getOptionLabel={(option: any) =>
                          `${option.nombres || option.nombre || ''} ${option.apellido || ''} (${option.nombre_usuario || option.email || ''})`
                        }
                        renderInput={(params) => (
                          <TextField
                            {...params}
