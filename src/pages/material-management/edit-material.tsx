...
const MaterialEditLayout = () => {
  ...
  const get_client = async () => {
    if (!id) {
      notification.error('Error al obtener el vehículo');
      return;
    }
    const data = await FetchData('materiales/' + id, 'GET', null); // Obtener vehículo desde el backend
    if (data.detail) {
      notification.error(data.detail);
      return;
    }
    console.log(data);
    setClient(data); // Asignar vehículo
  ...
  const create_user = async (e: event) => {
    ...
    const fetchdata = await FetchData('materiales/' + id, 'PUT', data);
    if (fetchdata.detail) {
      ...
      <Container>
        <div className="card min-w-full">
          <div className="card-header">
            <h3 className="card-title">Editar Material</h3>
