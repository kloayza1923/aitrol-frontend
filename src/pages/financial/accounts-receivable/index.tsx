...
export default function AccountsReceivable() {
  const columns: GridColDef[] = [
    { field: 'id', headerName: 'ID', width: 80 },
    {
      ...
      flex: 1,
      minWidth: 120,
      valueGetter: (params: any) => params?.row?.venta_numero ?? params?.row?.id_venta ?? ''
    },
    {
      ...
      valueGetter: (params: any) =>
        params?.row?.cliente_nombre ?? params?.row?.cliente?.nombre ?? params?.row?.id_cliente ?? ''
    },
    ...
    { field: 'fecha_vencimiento', headerName: 'Vencimiento', flex: 1, minWidth: 140 },
    {
      ...
      valueFormatter: (params: any) =>
        Number(params.value || 0).toLocaleString(undefined, {
  ...
  const handleProcess = async (row: any) => {
    try {
      // preview primero
      const previewData = await FetchData.post(`fin/cxc/${row.id}/process`, {
        auto: true,
        id_usuario: 1,
        preview: true
      });
      ...
      console.error(e);
      toast.error(e.message || 'Error al obtener previsualización');
    }
  };
  ...
  const handleConfirmProcess = async () => {
    ...
    try {
      const data = await FetchData.post(`fin/cxc/${previewRow.id}/process`, {
        auto: true,
        id_usuario: 1
      });
      toast.success('Contabilizado correctamente');
      ...
      console.error(e);
      toast.error(e.message || 'Error al contabilizar');
    }
  };
  ...
  const handleSubmitPayment = async () => {
    ...
    try {
      const data = await FetchData.post(`fin/cxc/${selectedRow.id}/apply-payment`, {
        monto,
        id_usuario: 1,
      ...
      console.error(e);
      toast.error(e.message || 'Error aplicando pago');
    }
  };
  ...
  return (
    <Container>
      ...
      />
      {/* Preview modal for contabilization */}
